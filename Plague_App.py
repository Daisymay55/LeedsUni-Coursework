...
def deriv(y, t, r0_y_interpolated, population, gamma, delta, beta, alpha, rho):
    S, E, I, R, D = y
...
#the variables for the equations above (alpha,beta,gamma,delta & rho)
...
#gamma
f= [i for i in range(4,10)]
for _ in range(1):
    gamma = float(random.choice(f))
    gamma = round((1/gamma),1)

#delta    
w= [i for i in range(1,7)]
for _ in range(1):
    delta = float(random.choice(w))
    delta = round((1/delta),1)
...
def Model(initial_cases, initial_date, population, R_0_start, k, x0, R_0_end, r0_y_interpolated=None):
    days = 274
    def beta(t):
    ...
    y0 = population-initial_cases, initial_cases, 0.0, 0.0, 0.0
    t = np.linspace(0, days, days)
    print(t)
    ret = odeint(deriv, y0, t, args=(r0_y_interpolated, population,
                                        gamma, delta, beta, alpha, rho))
    S, E, I, R, D = ret.T
